...
describe('#createMenu', () => {
  ...
  assert.deepStrictEqual(createMenu({'coxinha': 3.9, 'sopa': 9.9}).consumption, []);
    ...
    // ```
  const testando = createMenu();
  testando.order("coxinha");
  assert.deepStrictEqual(testando.consumption, ["coxinha"]);
  ...
  testando.order("agua");
  testando.order("sopa");
  testando.order("sashimi");
  assert.deepStrictEqual(testando.consumption, ["coxinha", "agua", "sopa", "sashimi"]);
