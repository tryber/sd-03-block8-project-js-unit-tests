...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const meuRestaurante = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} });
    assert.ok(meuRestaurante, typeof meuRestaurante === 'object');
    ...
    assert.deepEqual(Object.keys(meuRestaurante.fetchMenu), ['food', 'drinks']);
    ...
    assert.deepEqual(createMenu(objParam).fetchMenu,objParam);
    ...
    // ```
    assert.deepEqual(createMenu(objParam).consumption,[]);
    ...
    // ```
    assert.deepEqual(meuRestaurante.order('coxinha'), 'coxinha');
    ...
    meuRestaurante2.order(['coxinha','agua','sopa','sashimi']);
    assert.equal(meuRestaurante2.consumption.length,  4);
    ...
    // ```
    assert.ok(meuRestaurante2.order(['coxinha','agua','coxinha']));
    ...
    assert.equal(meuRestaurante3.pay(), 26.6);
